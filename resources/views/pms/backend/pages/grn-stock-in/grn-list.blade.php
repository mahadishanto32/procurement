                                ...
                                @foreach($purchaseOrder as $pkey=> $po)
                                    ...
                                    <tr>
                                        ...
                                        <td rowspan="{{ $po->relGoodReceiveNote->count() }}">
                                            <a href="javascript:void(0)" class="btn btn-link showGRNPODetails" data-src="{{route('pms.purchase.order-list.show',$values->relPurchaseOrder->id)}}" data-title="Purchase Order Details">{{$values->relPurchaseOrder->reference_no}}
                                            </a>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="javascript:void(0)" class="btn btn-link showGRNPODetails" data-src="{{route('pms.grn.grn-process.show',$values->id)}}" data-title="Gate-In Details">{{$values->reference_no}}
                                            </a>
                                        </td>
                                        <td>
                                            
                                            {{$values->grn_reference_no}}
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            <a class="btn btn-success btn-xs" href="{{ url('pms/grn-slip/'.$values->id) }}" target="_blank"><i class="la la-print"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                    @endif
