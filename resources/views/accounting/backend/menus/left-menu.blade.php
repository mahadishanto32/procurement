            ...
            <ul id="iq-sidebar-toggle" class="iq-menu">
                ...
                @forelse($menus as $key=>$menu)
                    ...
                    <li class="iq-menu-main {{$firstLiActiveClass}}">
                        ...
                        ?>
                        <a href="{{$url}}" class="iq-waves-effect {{$collapsed}}" data-toggle="{{$menuToggle}}" target="{{$menu->open_new_tab==App\Models\PmsModels\Accounts\Menu\AccountsMenu::OPEN_NEW_TAB?'_blank':''}}" aria-expanded="false"><i class="{{$menu->icon_class}}"></i><span>{{ __($menu->name) }}</span><i class="{{$dropDownIcon}}"></i></a>
                                ...
                                @foreach($menu->activeSubMenu as $subMenu)
                                    ...
                                    @can(json_decode($subMenu->slug,true))
                                        <li class="{{ Request()->path() == $subMenu->url?'active':'' }}">
                                            <a  href="{{URL::to($subMenu->url)}}"><i class="{{ !empty($subMenu->icon_class) ? $subMenu->icon_class : 'las la-arrow-right' }}" target="{{$subMenu->open_new_tab==App\Models\PmsModels\Accounts\Menu\AccountsSubMenu::OPEN_NEW_TAB?'_blank':''}}"></i>{{ __($subMenu->name) }}</a>
                                        </li>
                                    @endcan
