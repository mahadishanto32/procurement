        ...
        <div class="page-content">
            <div class="panel panel-info mt-3">
                <div class="panel-boby p-3">
                    <form action="{{ route('accounting.chart-of-accounts.update', $account->id) }}" method="post" accept-charset="utf-8">
                    @csrf
                    @method('PUT')
                        <div class="row pr-3">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group input-group-md mb-3 d-">
                                    <input type="text" name="code" id="code" value="{{ old('code', $account->code) }}" class="form-control rounded">
                                </div>
                            </div>
                            <div class="col-md-5">
                                <label for="name"><strong>{{ __('Ledger Name') }}:<span class="text-danger">&nbsp;*</span></strong></label>
                                <div class="input-group input-group-md mb-3 d-">
                                    <input type="text" name="name" id="name" value="{{ old('name', $account->name) }}" class="form-control rounded">
                                </div>
                            </div>
                        </div>
                        <div class="row pt-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="input-group input-group-md mb-3 d-">
                                        <select name="type" id="type" class="form-control rounded">
                                            <option value="D" {{ $account->type == "D" ? 'selected' : '' }}>Debit</option>
                                            <option value="C" {{ $account->type == "C" ? 'selected' : '' }}>Credit</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    ...
                                    <div class="input-group input-group-md mb-3 d-">
                                        <input type="number" step="0.01" name="opening_balance" id="opening_balance" value="{{ old('opening_balance', $account->opening_balance) }}" class="form-control text-right rounded">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <div class="input-group input-group-md mb-3 d-">
                                        <textarea name="notes" id="notes" class="form-control rounded" style="min-height: 120px">{{ old('notes', $account->notes) }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12 text-right">
                                ...
                                <button type="submit" class="btn btn-success btn-md"><i class="la la-save"></i>&nbsp;Update Ledger Account</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    function getCode() {
        $.ajax({
            url: '{{ url('accounting/chart-of-accounts') }}/'+$('#account_group_id').val()+"?account_id={{ $account->id }}",
            type: 'GET',
            data: {},
        })
        .done(function(code) {
